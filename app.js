...
function inspectSpace(n){sound('roll');$('space-number').textContent=n;$('space-number').classList.toggle('stamped',state.stamps.has(n));$('space-dialog').setAttribute('aria-label',`Board space ${n} effects`);$('space-dialog').showModal();}
function inspect(n){sound('roll');$('ball-detail').replaceChildren();const b=ball(n);b.tabIndex=-1;b.setAttribute('aria-label',`Ball ${n}; no powers`);$('ball-detail').append(b);$('ball-dialog').showModal();}
function refreshBag(){$('bag-grid').replaceChildren();for(const n of state.bag){const b=ball(n);if(state.stamps.has(n))b.classList.add('stamped-ball');if(state.offer.includes(n))b.classList.add('on-track');b.setAttribute('aria-label',`Ball ${n}${state.stamps.has(n)?', stamped on card':''}${state.offer.includes(n)?', on track':''}; played ${state.played[n]} times`);b.onclick=()=>inspect(n);$('bag-grid').append(b);}}
function lock(){busy=true;render();document.querySelectorAll('#balls .ball').forEach(b=>b.disabled=true);}
...
$('redraw').onclick=async()=>{if(busy||!state.redraws)return;lock();sound('roll');document.querySelectorAll('#balls .ball').forEach(b=>b.className='ball leave');await wait(480);redraw(state);render();showBalls();await wait(770);unlock();};
$('bag').onclick=()=>{refreshBag();$('bag-dialog').showModal();};$('stages').onclick=showStages;
for(const dialog of document.querySelectorAll('dialog:not(#result-dialog)')){dialog.querySelector('.close').onclick=()=>dialog.close();dialog.addEventListener('pointerdown',e=>{if(e.target!==dialog)return;const r=dialog.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)dialog.close();});}
nextDraw();
