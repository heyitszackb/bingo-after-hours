...
The main screen has a compact header with menu, centered current score / target, and money, the card rack, board, and a swipe lane with plays and passes remaining. The rack starts with Bingo, Face Value, and three blank placeholders; it fits additional owned cards without changing purchase limits. The score and target stay in the same top-center position throughout each round. During scoring, the entire scoring set lights up with Bingo, then each tile activates its points cards in order and sends its total into that persistent score. The counter reacts as each contribution arrives; the target stays visible. No sound is generated. The pause summary retains round progress; stage and pattern viewers remain hidden.
