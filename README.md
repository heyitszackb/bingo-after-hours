...
### Four stamp slots and tile inspection
Each tile records its individual stamps and accepts up to four. A fifth placement is rejected without spending a play, consuming the item, or creating a copy. New stamp destinations exclude full tiles; balls still occupy a full-stamp tile normally. Copier plus ×2 retains both effects and copies the incoming ×2 before consuming it. Each installed Copier creates one copy, while multipliers multiply together. Tile inspection lists the current item/effects, every stamp in application order, used slots out of four, and combined multiplier. Stamp lists persist between rounds/reloads; older saves reconstruct stamp lists from existing ink. Existing legacy ink is preserved. `node scripts/stamp-slots-check.mjs` verifies the fourth stamp, copying, tooltip and reload.

### Flat bonus stamps
+10, +50 and +100 stamps are one-use shop/Debug items. They persist in a tile's four stamp slots and add their combined bonus to each scoring activation before the tile multiplier. They do not trigger scoring themselves. Copier can duplicate them normally. Example: a 20 with +50 and ×2 scores 140 before any additional card effects. The bonus ink pulses during scoring. Tile inspection omits “Nothing special” when installed stamps provide effects. `node scripts/bonus-stamps-check.mjs` verifies placement, occupied-tile details, scoring and rewards.
