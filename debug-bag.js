import {ballValue,ITEM_TYPES} from './game.js?v=b6982f04c981';
...
export function bagRecipe(state){
  ...
  for(const id of state.collection){
    const type=state.items[id]||'number',value=['potionGravity','potionTiny','potionGiant','potionPolish','potionChisel','potion20','potion2','potionCopy','potionMelt','potionSticky','doubleball','question','bomb','x2','x3','copier','trash','plus10','plus50','plus100'].includes(type)?null:type==='d20'?(state.valueModifiers[id]||0):ballValue(state,id),key=`${type}:${value}`;
...
export function buildDebugBag(recipe){
  ...
  for(const row of recipe){
    ...
    if(!['potionGravity','potionTiny','potionGiant','potionPolish','potionChisel','potion20','potion2','potionCopy','potionMelt','potionSticky','doubleball','question','bomb','x2','x3','copier','trash','plus10','plus50','plus100'].includes(row.type)&&!Number.isSafeInteger(row.value))throw new Error('Values must be whole numbers.');
    total+=row.count;
  ...
  for(const row of recipe)for(let i=0;i<row.count;i++){
    ...
    if(row.type==='number'){if(!baseId)ballValues[id]=row.value;}
      ...
      if((row.type==='hundred'&&row.value!==50)||(['seed','glass'].includes(row.type)&&row.value!==1)||(['earth','blackjack','moon','jupiter','king','tornado','prism','anchor','feather','crown','valley','oddball','sun'].includes(row.type)&&row.value!==0)||(row.type==='phoenix'&&row.value!==10))ballValues[id]=row.value;
    }
  }
