import {ballValue,ITEM_TYPES} from './game.js?v=fdaf9df0ff68';
...
export function buildDebugBag(recipe){
  ...
  for(const row of recipe)for(let i=0;i<row.count;i++){
    ...
    if(row.type==='number'){if(!baseId)ballValues[id]=row.value;}
      ...
      if((row.type==='hundred'&&row.value!==50)||(['seed','glass'].includes(row.type)&&row.value!==1)||(['earth','mars','moon','jupiter','king','tornado','prism','anchor'].includes(row.type)&&row.value!==0)||(row.type==='phoenix'&&row.value!==10))ballValues[id]=row.value;
    }
  }
