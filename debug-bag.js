import {ballValue,ITEM_TYPES} from './game.js?v=8a867063cf85';
...
export function buildDebugBag(recipe){
  ...
  for(const row of recipe)for(let i=0;i<row.count;i++){
    ...
    if(row.type==='number'){if(!baseId)ballValues[id]=row.value;}
      ...
      if((row.type==='hundred'&&row.value!==50)||(row.type==='seed'&&row.value!==1)||(row.type==='king'&&row.value!==10))ballValues[id]=row.value;
    }
  }
