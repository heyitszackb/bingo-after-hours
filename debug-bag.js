import {ballValue,ITEM_TYPES} from './game.js?v=0ad392a44205';
...
export function bagRecipe(state){
  ...
  for(const id of state.collection){
    const type=state.items[id]||'number',value=['bomb','rock','x2','x3','copier'].includes(type)?null:type==='d20'?(state.valueModifiers[id]||0):ballValue(state,id),key=`${type}:${value}`;
...
export function buildDebugBag(recipe){
  ...
  for(const row of recipe){
    ...
    if(!['bomb','rock','x2','x3','copier'].includes(row.type)&&!Number.isSafeInteger(row.value))throw new Error('Values must be whole numbers.');
    total+=row.count;
  ...
  for(const row of recipe)for(let i=0;i<row.count;i++){
    ...
    if(row.type==='number'){if(!baseId)ballValues[id]=row.value;}
      ...
      if((row.type==='hundred'&&row.value!==100)||(row.type==='seed'&&row.value!==1))ballValues[id]=row.value;
    }
  }
