import {ballValue,ITEM_TYPES} from './game.js?v=25ae7204469d';
...
export function bagRecipe(state){
  ...
  for(const id of state.collection){
    const type=state.items[id]||'number',value=['doubleball','question','bomb','rock','x2','x3','copier','trash','plus10','plus50','plus100'].includes(type)?null:type==='d20'?(state.valueModifiers[id]||0):ballValue(state,id),key=`${type}:${value}`;
...
export function buildDebugBag(recipe){
  ...
  for(const row of recipe){
    ...
    if(!['doubleball','question','bomb','rock','x2','x3','copier','trash','plus10','plus50','plus100'].includes(row.type)&&!Number.isSafeInteger(row.value))throw new Error('Values must be whole numbers.');
    total+=row.count;
  ...
  for(const row of recipe)for(let i=0;i<row.count;i++){
    ...
    if(row.type==='number'){if(!baseId)ballValues[id]=row.value;}
      ...
      if((row.type==='hundred'&&row.value!==50)||(row.type==='seed'&&row.value!==1)||(['king','statue'].includes(row.type)&&row.value!==10))ballValues[id]=row.value;
    }
  }
