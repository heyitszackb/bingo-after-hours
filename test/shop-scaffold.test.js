...
test('retired cards remain unavailable while High Five joins the catalog',()=>{
 assert.deepEqual(Object.keys(CARD_TYPES),['square','corners','crowd','silver-lining','full-sweep','encore','high-five']);assert.deepEqual(BALL_UPGRADES,{});retiredCards.forEach(id=>assert.equal(cardDetails(id),null));
