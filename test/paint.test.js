...
test('all scoring shapes use one base point, stack red and retain fixed gold/blue bonuses',()=>{
 for(const pattern of PATTERNS){const s=newStage(10,5,{25:'red',24:'red',23:'gold',22:'blue'});let r;for(let i=0;i<pattern.length;i++)r=place(s,25-i,pattern[i]);assert.equal(r.points,pattern.length*4);assert.ok(r.activations.every(a=>a.points===4));assert.equal(r.gold,1);assert.equal(r.bonusDraws,3);assert.equal(s.score,pattern.length*4);assert.equal(s.stamps.size,0);}
});
test('shared tile activates once per simultaneous pattern; only scored stamps clear',()=>{
 ...
 const r=place(s,25,1);assert.deepEqual(r.scoredPatterns.map(pattern=>pattern.type),['row','column','diagonal','corners','square4']);assert.equal(r.patterns.length,5);assert.equal(r.points,23);assert.equal(r.bonusDraws,15);assert.equal(r.activations.filter(a=>a.tile===1).length,5);assert.deepEqual([...s.stamps],[24]);assert.deepEqual(s.stampBalls,{24:13});assert.deepEqual(s.patternCounts,{row:1,column:1,diagonal:1,corners:1,square4:1});
});
...
test('black paint replaces other paint for $3 and keeps one-point base scoring',()=>{
