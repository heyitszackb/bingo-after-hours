...
test('all scoring shapes use ten base points per line, stack red and retain fixed gold/blue bonuses',()=>{
 for(const pattern of PATTERNS){const s=newStage(10,5,{25:'red',24:'red',23:'gold',22:'blue'});let r;for(let i=0;i<pattern.length;i++)r=place(s,25-i,pattern[i]);assert.equal(r.points,40);assert.ok(r.activations.every(a=>a.points===8));assert.equal(r.gold,1);assert.equal(r.bonusDraws,3);assert.equal(s.score,40);assert.equal(s.stamps.size,5);}
});
test('shared tile activates once per simultaneous pattern; all stamps remain',()=>{
 ...
 const r=place(s,25,1);assert.deepEqual(r.scoredPatterns.map(pattern=>pattern.type),['row','column','diagonal']);assert.equal(r.patterns.length,3);assert.equal(r.points,30);assert.equal(r.bonusDraws,9);assert.equal(r.activations.filter(a=>a.tile===1).length,3);assert.deepEqual([...s.stamps],[...positions,1]);assert.equal(s.stampBalls[24],13);assert.equal(s.stampBalls[1],25);assert.deepEqual(s.patternCounts,{row:1,column:1,diagonal:1});
});
