...
test('painting charges $3 only for valid purchases; replacing paint is allowed',()=>{
  ...
  assert.ok(paintBall(s,1,'red'));assert.equal(s.money,before-6);assert.equal(s.paints[1],'red');assert.ok(paintBall(s,2,'blue'));assert.equal(s.paints[2],'blue');
...
test('every bingo orientation applies red only when the pattern scores',()=>{
  for(const pattern of PATTERNS){
