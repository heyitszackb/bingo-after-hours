...
test('stage reset refreshes board calls and counts, increases target',()=>{const s=newStage(2);assert.equal(s.target,20);assert.equal(s.calls,12);assert.equal(s.money,5);assert.equal(s.stamps.size,0);assert.ok(s.played.every(n=>n===0));});
