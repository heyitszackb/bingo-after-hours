...
test('all remaining balls share the last empty tile',()=>{
 const s=newStage();s.bag=new Set([1,2]);assert.equal(deal(s).length,2);s.stamps=new Set(Array.from({length:24},(_,i)=>i+1));assert.equal(deal(s).length,2);assert.deepEqual(Object.values(s.destinations),[25,25]);
});
...
test('shop cards are free while empty ball-upgrade slots remain unavailable',()=>{
 ...
 assert.ok(s.shopOffer.cards.length>=2);assert.equal(new Set(s.shopOffer.cards).size,s.shopOffer.cards.length);assert.deepEqual(s.shopOffer.items,['tornado','prism','phoenix','anchor','potion20','potion2','potionCopy','potionMelt','doubleball','trash','potionSticky','glass','king','question','plus10','plus50','plus100','copier','x2','x3','seed','bomb','d20','moon','mars','earth','jupiter']);const cash=s.money;
