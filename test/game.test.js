...
test('every geometric pattern scores ten base points per line independent of ball numbers',()=>{
 for(const pattern of PATTERNS){const s=newStage(10);let r;for(let i=0;i<pattern.length;i++)r=place(s,25-i,pattern[i]);assert.equal(r.points,10);assert.deepEqual(r.activations.map(a=>a.tile),pattern);assert.deepEqual(r.activations.map(a=>a.number),pattern.map((_,i)=>25-i));assert.ok(r.activations.every(a=>a.points===2));assert.equal(s.stamps.size,0);assert.deepEqual(s.stampBalls,{});assert.equal(s.bag.size,25-pattern.length);}
});
