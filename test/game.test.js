...
test('shop cards are free while empty ball-upgrade slots remain unavailable',()=>{
 ...
 assert.ok(s.shopOffer.cards.length>=2);assert.equal(new Set(s.shopOffer.cards).size,s.shopOffer.cards.length);assert.deepEqual(s.shopOffer.items,['plus10','plus50','plus100','copier','x2','x3','seed','bomb','d20','hundred','rock']);const cash=s.money;
