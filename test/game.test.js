...
test('new stages are blank with all balls, 15 plays and 10 passes and no permanent mappings',()=>{
 const s=newStage();assert.equal(s.stamps.size,0);assert.deepEqual(s.stampBalls,{});assert.deepEqual(s.destinations,{});assert.equal(s.bag.size,25);assert.equal(s.calls,15);assert.equal(s.money,5);assert.equal(s.board,undefined);
});
...
test('playing uses the offered destination, removes only that ball, and clears offers',()=>{
 const s=newStage();s.offer=[1,2,3];s.destinations={1:25,2:7,3:9};const r=choose(s,1);assert.equal(r.tile,25);assert.deepEqual([...s.stamps],[25]);assert.equal(s.stampBalls[25],1);assert.equal(s.calls,14);assert.ok(!s.bag.has(1));assert.ok(s.bag.has(2));assert.equal(s.played[1],1);assert.deepEqual(s.offer,[]);assert.deepEqual(s.destinations,{});
});
test('invalid, stale and occupied destinations spend nothing',()=>{
 const s=newStage();place(s,1,7);s.offer=[2];s.destinations={2:7};assert.equal(choose(s,2),null);assert.equal(s.calls,14);assert.equal(s.bag.size,24);s.destinations={};assert.equal(choose(s,2),null);s.destinations={2:26};assert.equal(choose(s,2),null);assert.equal(choose(s,1),null);
});
...
test('passing spends a pass, redraws ball and space, and preserves money, plays, stamps and bag',()=>{
 ...
 assert.ok(redraw(s,()=>0));assert.notDeepEqual(s.destinations,first);assert.notEqual(s.offer[0],number);assert.equal(s.money,5);assert.equal(s.calls,14);assert.equal(s.passes,9);assert.equal(s.stampBalls[1],25);assert.deepEqual([...s.bag],bag);assert.equal(s.played[number],0);
 for(let i=0;i<9;i++)assert.ok(redraw(s));const offer=[...s.offer],dest={...s.destinations};assert.equal(s.passes,0);assert.equal(s.status,'playing');assert.equal(redraw(s),false);assert.deepEqual(s.offer,offer);assert.deepEqual(s.destinations,dest);assert.ok(choose(s,s.offer[0]));assert.equal(s.calls,13);
});
test('passes work without money, refill each round, and cannot be spent after a run ends',()=>{
 ...
 const next=newStage(2,s.money,s.upgrades,s.patternCounts,s.jokers);assert.equal(next.calls,15);assert.equal(next.passes,10);
});
...
test('shop cards are free while empty ball-upgrade slots remain unavailable',()=>{
 ...
 assert.ok(s.shopOffer.cards.length>=2);assert.equal(new Set(s.shopOffer.cards).size,s.shopOffer.cards.length);assert.deepEqual(s.shopOffer.items,['question','plus10','plus50','plus100','copier','x2','x3','seed','bomb','d20','hundred','rock']);const cash=s.money;
