...
test('last-call success precedes game over; failure ends run',()=>{const s=newStage();s.score=5;s.calls=1;s.stamps=new Set([1,2,3,4]);s.offer=[5];choose(s,5);assert.equal(s.status,'passed');const f=newStage();f.calls=1;f.offer=[2];choose(f,2);assert.equal(f.status,'over');assert.equal(choose(f,2),null);});
