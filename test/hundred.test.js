...
test('100 Ball is free, independently drawable and passing never changes values',()=>{
 const {s,id}=fixture();assert.equal(ballValue(s,id),100);assert.equal(s.money,0);s.bag=new Set([1,id]);let hits=0;for(let i=0;i<100;i++)if(draw(s,()=>i/100)[0]===id)hits++;assert.equal(hits,50);
 stamp(s,2,8);s.offer=[id];s.destinations={[id]:13};redraw(s,()=>.5);assert.equal(s.stampValues[8],2);assert.ok(s.bag.has(id));assert.equal(s.calls,15);assert.deepEqual(s.valueModifiers,{});
});
...
test('completed lines use reduced numbers immediately, including overlapping lines',()=>{
 ...
 assert.equal(r.scoringGroups.length,2);assert.equal(r.points,232);assert.equal(r.valueChanges.length,4);assert.equal(r.activations.find(a=>a.tile===12).number,1);assert.equal(r.activations.find(a=>a.tile===8).number,5);
});
...
test('a die retains its permanent penalty on future rolls; other 100 Balls can be reduced',()=>{
 const {s,id,other,die}=fixture();stamp(s,die,13,1);stamp(s,other,9);play(s,id,8);assert.equal(s.stampValues[13],0);assert.equal(s.stampValues[9],99);
 const next=newStage(10,0,{}, {},s.jokers,s);assert.equal(ballValue(next,die),null);assert.equal(ballValue(next,other),99);const r=play(next,die,13,()=>0);assert.equal(r.roll,0);assert.equal(next.stampValues[13],0);
});
