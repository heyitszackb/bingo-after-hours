...
function finish(jokers,occupied,tile){const s=newStage(10,5,{}, {},jokers);s.target=10000;for(const t of occupied){s.stamps.add(t);s.stampBalls[t]=t;s.stampValues[t]=t;s.bag.delete(t);}s.offer=[tile];s.destinations={[tile]:tile};return {s,r:choose(s,tile)};}
...
test('corners plus Bingo work together; stamp ink does not occupy a corner and absent cards do not trigger',()=>{assert.equal(finish(['bingo','corners:1','face-value'],[1,2,3,4,21,25],5).r.points,76);assert.equal(finish(['face-value'],[1,5,21],25).r.points,0);const {s}=finish(['corners:1','face-value'],[1,5],21);s.tileMultipliers[25]=3;s.offer=[20];s.destinations={20:20};assert.equal(choose(s,20).points,0);});
test('Encore repeats the Square condition, but unrelated future placements do not repeat completed patterns',()=>{const {s,r}=finish(['encore:1','square:1','face-value'],[1,2,6],7);assert.equal(r.points,32);assert.equal(s.patternCounts.square,1);s.offer=[20];s.destinations={20:20};assert.equal(choose(s,20).points,0);assert.deepEqual(newStage(10,5,{}, {},s.jokers,s).scoredLines,[]);});
...
test('Four Corners scores all occupied tiles with Face Value and Encore, only once per round',()=>{
 ...
 s.offer=[2];s.destinations={2:2};assert.equal(choose(s,2).points,0);
