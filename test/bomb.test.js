...
test('Bomb destroys itself and adjacent items before a newly completed line can score',()=>{
 ...
 const r=play(s,id,5);assert.equal(r.points,0);assert.deepEqual(r.activations,[]);assert.deepEqual(r.scoredPatterns,[]);assert.ok(r.effectBoard.stamps.has(5));assert.equal(r.effectBoard.stampBalls[5],id);assert.deepEqual(r.destroyed.map(d=>d.id).sort((a,b)=>a-b),[4,7,id]);
...
test('center blast removes four orthogonal neighbors; emptied board spaces remain playable',()=>{
 const {s,id}=withBomb(),neighbors=[7,8,9,12,14,17,18,19];neighbors.forEach((t,i)=>stamp(s,i+1,t));stamp(s,20,1);const r=play(s,id,13);
 assert.equal(r.destroyed.length,5);assert.deepEqual([...s.stamps],[7,9,17,19,1]);assert.equal(s.collection.length,21);assert.equal(s.bag.size,16);assert.ok(play(s,21,13));assert.equal(s.stampBalls[13],21);
});
test('corner blast does not wrap across rows; no-line Bomb still explodes',()=>{
 ...
 assert.equal(r.activations.length,0);assert.equal(r.points,0);assert.deepEqual(new Set(r.destroyed.map(d=>d.id)),new Set([2,6,id]));assert.equal(s.stampBalls[5],5);
});
