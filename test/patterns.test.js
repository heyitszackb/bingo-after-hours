...
test('only five rows, five columns and two full diagonals score, each worth ten',()=>{
 ...
 for(const p of PATTERN_DEFINITIONS){
  ...
  assert.equal(result.points,10);assert.equal(s.stamps.size,0);assert.equal(s.patternCounts[p.type],1);
 }
});
...
test('overlapping row and column score together; a removed diagonal stays except for shared cleared stamps',()=>{
 ...
 assert.equal(r.activations.length,10);assert.deepEqual([...s.stamps],[7,13,19,25]);
