...
test('base preset exactly recreates the original 1–25 collection',()=>{
 const inventory=buildDebugBag(baseBagRecipe()),s=make(baseBagRecipe());assert.deepEqual(inventory.collection,Array.from({length:25},(_,i)=>i+1));assert.equal(inventory.nextItemId,26);assert.deepEqual(inventory.items,{});assert.deepEqual(bagRecipe(s),baseBagRecipe());assert.equal(s.calls,15);assert.equal(s.passes,10);
});
