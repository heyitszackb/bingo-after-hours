...
test('Number Cruncher adds current values to the existing base and other bonuses; X adds no number',()=>{
 const s=newStage(10,5,{25:'x'}, {},['bingo','face-value','number-cruncher:1','single-digits:1']);
 [5,19,23,1].forEach((n,i)=>stamp(s,n,i+1));const r=place(s,25,5);
 assert.equal(r.points,98);assert.equal(r.activations[4].points,0);assert.deepEqual(r.activations[4].bonuses,[]);
 const d=newStage(10,5,{25:'doubler'}, {},['bingo','face-value','number-cruncher:1']);[1,2,3,4].forEach(n=>stamp(d,n,n));
 assert.equal(place(d,25,5).points,78);assert.equal(d.stampValues[4],8);
});
test('Second Wind uses the inclusive five-number range and current values, excludes X, rescues last call',()=>{
 const s=newStage(10,5,{25:'x'}, {},['bingo','face-value','call-range:13:1']);
 [13,17,12,18].forEach((n,i)=>stamp(s,n,i+1));s.calls=1;const r=place(s,25,5);
 assert.equal(r.points,60);assert.equal(s.calls,2);assert.equal(s.status,'playing');assert.equal(r.callsBeforeBonuses,0);
...
test('call bonuses cap at 15 including stacked copies and overlapping lines',()=>{
 const s=newStage(10,5,{}, {},['bingo','face-value','call-range:1:1','call-range:1:2']);
 ...
 const cross=newStage(10,5,{}, {},['bingo','face-value','call-range:1:1']);cross.calls=1;
...
test('shops have unique card types; ranges roll once per offer, persist on purchase and through stages',()=>{
 ...
 const next=newStage(2,s.money,s.upgrades,s.patternCounts,s.jokers);assert.equal(cardDetails(next.jokers[3]).start,13);assert.equal(cardDetails(next.jokers[3]).end,17);
