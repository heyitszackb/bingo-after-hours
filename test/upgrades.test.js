...
test('X can occupy any empty tile, has no number, but completes lines and earns positional bonuses',()=>{
 const s=newStage(10,5,{5:'x'}, {},['bingo','face-value','single-digits:1','outer-layer:1']);[1,2,3,4].forEach(n=>stamp(s,n,n));s.offer=[5];s.destinations={5:25};
 const r=choose(s,5,5);assert.equal(ballValue(s,5),null);assert.equal(s.stampValues[5],null);assert.equal(r.points,39);
...
test('Doubler changes all eight neighbors, not itself, distant tiles, or X; eligibility uses the new value',()=>{
 const s=newStage(10,5,{25:'doubler',2:'x'}, {},['bingo','face-value','single-digits:1']);
...
test('doubling happens before Face Value and numeric bonus eligibility',()=>{
 const s=newStage(10,5,{25:'doubler'}, {},['bingo','face-value','single-digits:1']);[1,2,3,4].forEach(n=>stamp(s,n,n,n===4?5:n));
 const r=place(s,25,5);assert.equal(s.stampValues[4],10);assert.equal(r.points,44);assert.equal(r.activations[3].bonuses.length,0);
});
test('Tornado scatters every stamp without losing identity or values, then scores its resulting lines',()=>{
 ...
 assert.deepEqual(Object.values(s.stampValues),[null,8,12,16,25]);assert.equal(r.points,61);assert.equal(s.calls,14);assert.equal(s.bag.size,20);
...
test('Outer Layer follows positions and stacks independently with single digits and duplicate cards',()=>{
 const s=newStage(10,5,{}, {},['bingo','face-value','outer-layer:1','outer-layer:2','single-digits:1']);let r;
 [11,12,13,14,15].forEach((t,i)=>{r=place(s,i+1,t);});assert.equal(r.points,40);
 assert.deepEqual(r.activations.map(a=>a.points),[12,3,4,5,16]);
});
test('five purchased cards plus both starting rules; trashing one copy frees a slot without removing another',()=>{
 ...
 assert.equal(s.jokers.length,7);s.shopOffer=null;openShop(s);const money=s.money;assert.equal(buyCard(s,0),false);assert.equal(s.money,money);
 const id=s.jokers[2];removeJoker(s,id);assert.ok(buyCard(s,0));assert.equal(s.jokers.length,7);assert.equal(new Set(s.jokers).size,7);
});
