...
const assets={gold:'coin.svg',red:'ball-red.svg',blue:'ball-blue.svg',black:'ball-black.svg'};
for(const [i,color] of ['gold','red','blue','black'].entries()){
 const n=[1,7,13,1][i],paint=page.locator(`[data-paint=${color}]`),ball=page.locator(`#shop-balls [data-number="${n}"]`);
...
await page.locator('#shop-bag').click();for(const [n,color] of [[1,'black'],[7,'red'],[13,'blue']])assert.ok((await page.locator(`#bag-grid [data-number="${n}"]`).evaluate(b=>getComputedStyle(b).backgroundImage)).includes(assets[color]));await page.locator('#bag-dialog .close').click();
...
const backgrounds=[];
for(const [n,color] of [[1,'black'],[7,'red'],[13,'blue']]){const tile=page.locator(`#cell-${saved.destinations[n]}`);assert.ok(await tile.evaluate((c,color)=>c.classList.contains('paint-grey'),color));backgrounds.push(await tile.evaluate(c=>getComputedStyle(c).backgroundColor));}
...
for(const [n,color] of [[1,'black'],[7,'red'],[13,'blue']]){
 ...
 const b=await page.locator('#balls .ball').boundingBox(),board=await page.locator(color==='black'?`#cell-${saved.destinations[n]}`:'#board').boundingBox();
