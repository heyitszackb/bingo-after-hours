...
try{
 ...
 // Dynamite animates a complete permutation and retains all identities and values.
 const dy=newStage(10,5,{25:'dynamite',1:'x'});[6,12,18,24].forEach((tile,i)=>{dy.stamps.add(tile);dy.stampBalls[tile]=i+1;dy.stampValues[tile]=i===0?null:(i+1)*4;dy.bag.delete(i+1);});dy.offer=[25];dy.destinations={25:13};await restore(dy);await ready();await page.locator('#balls .ball').focus();await page.keyboard.press('Space');await page.waitForFunction(()=>!document.querySelector('#pause-button').disabled&&document.querySelector('#calls').textContent==='11');
 ...
 assert.deepEqual(errors,[]);console.log('Passed mixed shop, prices, five-card cap, sold/save state, touch trash, X placement, Doubler values, Dynamite identities, migration and responsive layouts.');
