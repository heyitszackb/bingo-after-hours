...
await page.locator('#stages').tap();assert.deepEqual(await page.locator('.stage-node small').allTextContents(),['5 pts','10 pts','20 pts','40 pts','100 pts','200 pts','500 pts','1,000 pts','5,000 pts','10,000 pts']);await page.locator('#stage-dialog .close').tap();
...
const late=await browser.newPage({viewport:{width:320,height:568},reducedMotion:"reduce"});await late.addInitScript(s=>localStorage.setItem('binglatro.run.v1',JSON.stringify(s)),saved);await late.goto(url);await late.locator('#play-button').click();await late.locator('#balls .ball:not([disabled])').first().waitFor();assert.equal(await late.locator('#target').textContent(),'10000');await late.close();
