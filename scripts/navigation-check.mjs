...
await page.locator('#play-button').tap();await page.locator('#balls .ball:not([disabled])').first().waitFor();assert.equal(await page.locator('#target').textContent(),'40');
const n=await page.locator('#balls .face').first().textContent();await page.locator('#balls .ball').first().focus();await page.keyboard.press('Space');await page.waitForFunction(()=>document.querySelector('#calls').textContent==='11'&&!document.querySelector('#pause-button').disabled);
...
assert.equal(await page.locator('#calls').textContent(),'11');assert.equal(await page.locator('#money').textContent(),'4');assert.deepEqual(await page.locator('#balls .face').allTextContents(),offer);assert.ok(await page.locator(`#cell-${n}`).evaluate(c=>c.classList.contains('stamped')));
...
await page.locator('#stages').tap();assert.deepEqual(await page.locator('.stage-node small').allTextContents(),['40 pts','70 pts','100 pts','140 pts','190 pts','250 pts','325 pts','425 pts','550 pts','700 pts']);await page.locator('#stage-dialog .close').tap();
...
const late=await browser.newPage({viewport:{width:320,height:568},reducedMotion:"reduce"});await late.addInitScript(s=>localStorage.setItem('binglatro.run.v1',JSON.stringify(s)),saved);await late.goto(url);await late.locator('#play-button').click();await late.locator('#balls .ball:not([disabled])').first().waitFor();assert.equal(await late.locator('#target').textContent(),'700');await late.close();
