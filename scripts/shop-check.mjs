...
// Two orange paints multiply a row by four, visibly scoring 4 per square.
const orange=newStage(4,5,{1:'orange',3:'orange'});orange.stamps=new Set([1,2,3,4]);orange.offer=[5,6,7];await restore(orange);await page.locator('#balls .ball:not([disabled])').first().waitFor();
...
await drag(page.locator('#balls [data-number="5"]'),page.locator('#board'));await page.locator('.row-multiplier').waitFor({state:'visible'});assert.equal(await page.locator('.row-multiplier').textContent(),'×4');await page.locator('#balls .ball:not([disabled])').first().waitFor();assert.equal(await page.locator('#score').textContent(),'20');assert.deepEqual(await page.evaluate(()=>[...new Set(window.scoreSteps.filter(n=>n>0))]),[4,8,12,16,20]);
...
assert.deepEqual(errors,[]);await browser.close();console.log('Passed mobile shop fit, touch/mouse paint drag, cancelled/same-color drops, prices, refresh, bag colors, saved shop, paint persistence, gold coin flights, orange row scoring, and keyboard purchasing.');
