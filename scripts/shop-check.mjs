...
async function restore(s){await page.goto(url);await page.evaluate(s=>sessionStorage.setItem('fixture',JSON.stringify(s)),{...s,bag:[...s.bag].filter(n=>![...s.stamps].map(t=>s.stampBalls[t]??t).includes(n)),stamps:[...s.stamps],stampBalls:Object.fromEntries([...s.stamps].map(t=>[t,s.stampBalls[t]??t])),destinations:Object.fromEntries(s.offer.map(n=>[n,s.destinations[n]??n]))});await page.reload();await page.locator('#play-button').click();}
...
await restore(s);await page.locator('#shop-screen').waitFor({state:'visible'});await page.locator('#shop-screen').evaluate(el=>Promise.all(el.getAnimations().map(a=>a.finished.catch(()=>{}))));
