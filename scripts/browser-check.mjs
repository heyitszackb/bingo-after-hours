...
await page.locator('#cell-13').tap();assert.equal(await page.locator('#tooltip-number').textContent(),'13');assert.ok(await page.locator('#tooltip-effect').isVisible());assert.equal(await page.locator('#tooltip-effect').textContent(),'Nothing special');assert.equal(await page.locator('dialog[open]').count(),0);
await page.locator('#cell-25').focus();await page.keyboard.press('Enter');assert.equal(await page.locator('#tooltip-number').textContent(),'25');await page.keyboard.press('Escape');assert.ok(await page.locator('#inspect-tooltip').isHidden());
await first.tap();
...
assert.ok(await page.locator(`#cell-${n}`).evaluate(e=>e.classList.contains('stamped')));assert.equal(await page.locator('.drag-ghost').count(),0);assert.ok(await page.locator('#inspect-tooltip').isHidden());
await page.locator('#bag').tap();assert.equal(await page.locator('#bag-grid .ball').count(),24);assert.equal(await page.locator('#bag-count').textContent(),'24');assert.ok(!(await page.locator('#bag-grid .face').allTextContents()).includes(n));assert.equal(await page.locator('#bag-grid .on-track').count(),3);await page.locator('#bag-grid .ball').first().tap();assert.ok(await page.locator('#inspect-tooltip').isVisible());assert.equal(await page.locator('dialog[open]').count(),1);await page.keyboard.press('Escape');assert.ok(await page.locator('#bag-dialog').isVisible());await page.locator('#bag-dialog .close').tap();
...
await page.locator('#redraw').tap();await ready();assert.equal(await page.locator('#calls').textContent(),'11');assert.equal(await page.locator('#redraw-count').textContent(),'1');assert.ok(!(await page.locator('#balls .face').allTextContents()).includes(n));assert.equal(await page.locator('#bag-count').textContent(),'24');
