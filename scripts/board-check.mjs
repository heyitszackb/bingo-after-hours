...
await page.locator('#shop-screen').waitFor({state:'visible'});assert.equal(await page.locator('#score').textContent(),'10');assert.equal(await page.locator('#money').textContent(),'20');assert.equal(await page.locator('.cell.stamped').count(),0);assert.equal((await page.locator('#board .cell span').allTextContents()).filter(Boolean).length,0);
