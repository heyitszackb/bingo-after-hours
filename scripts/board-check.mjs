...
let state=await read();assert.equal((await page.locator('#board .cell span').allTextContents()).filter(Boolean).length,3);assert.equal(await page.locator('.cell.offered').count(),3);assert.equal(await page.locator('.cell.stamped').count(),0);
for(const n of state.offer)assert.equal(await page.locator(`#cell-${state.destinations[n]} span`).textContent(),String(n));
...
assert.equal(await page.locator('#cell-1 span').textContent(),'');await page.screenshot({path:'/tmp/binglatro-blank-board.png'});
const a=await page.locator('#balls [data-number="3"]').boundingBox(),b=await page.locator('#board').boundingBox();await page.mouse.move(a.x+a.width/2,a.y+a.height/2);await page.mouse.down();await page.mouse.move(b.x+b.width-10,b.y+10,{steps:12});assert.ok(await page.locator('#cell-21').evaluate(c=>c.classList.contains('destination')));await page.mouse.up();
await page.locator('.pattern-multiplier').waitFor({state:'visible'});assert.deepEqual(await page.locator('.cell.multiplied').evaluateAll(c=>c.map(n=>Number(n.id.replace('cell-','')))),[1,6,11,16,21]);assert.equal((await page.locator('#board .cell span').allTextContents()).filter(Boolean).length,0);
await page.locator('#shop-screen').waitFor({state:'visible'});assert.equal(await page.locator('#score').textContent(),'10');assert.equal(await page.locator('#money').textContent(),'20');assert.equal(await page.locator('.cell.stamped').count(),0);
...
assert.deepEqual(errors,[]);await browser.close();console.log('Passed blank tiles, random per-offer assignments, fixed reorder/resume mapping, destination snapping, placement-based paint scoring, stage reset and old-save migration.');
