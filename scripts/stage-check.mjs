...
for(const start of [1,10]){
 ...
 await page.route('**/game.js',route=>route.fulfill({contentType:'text/javascript',body:source.replace('stage=1','stage='+start).replace('target:targetFor(stage)','target:10').replace('stamps:new Set()','stamps:new Set([1,2,3,4])').replace('return bag.slice(0,Math.min(count,25))','return [5,6,7]')}));
 ...
 assert.equal(await page.locator('#score').textContent(),'10');assert.equal(await page.locator('.cell.stamped').count(),0);
