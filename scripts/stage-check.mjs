...
for(const start of [1,10]){
 ...
 await page.goto('http://localhost:5173');await page.locator('#play-button').click();await page.locator('#balls .ball:not([disabled])').first().waitFor();
 ...
 await page.locator('#continue:not([disabled])').waitFor();
 assert.equal(await page.locator('#payout-count').textContent(),'11');
 assert.equal(await page.locator('#payout-earned').textContent(),'+$11');
 assert.equal(await page.locator('#payout-money').textContent(),'$15');
 assert.equal(await page.locator('#money').textContent(),'15');
 assert.equal(await page.locator('.payout-coin.cashed').count(),11);
 ...
 if(start===1){await page.locator('#result-menu').click();await page.locator('#play-button').click();await page.locator('#continue:not([disabled])').waitFor();assert.equal(await page.locator('#money').textContent(),'15');}
