...
try{
 for(const touch of [false,true]){
  ...
  const state=newStage(10,5,{1:'red',2:'gold',3:'blue'});state.offer=[1,2,3];state.destinations={1:1,2:13,3:25};
  ...
  for(const [space,nearest] of [[1,1],[25,25],[10,13]]){
  ...
  assert.equal(await page.locator('#cell-13 span').textContent(),'1');
  assert.ok(await page.locator('#cell-13').evaluate(c=>c.classList.contains('paint-red')));
  ...
  assert.equal(await page.locator('#cell-13 span').textContent(),'1');
  if(touch)await page.screenshot({path:'/tmp/binglatro-numbered-stamp.png'});
