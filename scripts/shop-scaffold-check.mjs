...
try{
 ...
 assert.equal(await page.locator('#shop-cards .card-slot').count(),2);assert.equal(await page.locator('#shop-balls .ball-slot').count(),2);assert.equal(await page.locator('#shop-screen button').count(),1);assert.equal(await page.locator('#shop-redraw,#upgrade-dialog,#plasma-dialog,#plasma-pick').count(),0);
