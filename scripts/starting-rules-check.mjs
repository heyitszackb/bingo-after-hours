...
try{
 ...
 await page.addInitScript(()=>{
  ...
  Element.prototype.animate=function(frames,options){
   if(this.matches('.joker-card.scoring-card'))window.scoreEvents.push({id:this.dataset.joker,payout:this.dataset.payout,tiles:[...document.querySelectorAll('.cell.pattern-active')].map(c=>c.id),active:[...document.querySelectorAll('.cell.activating')].map(c=>c.id),at:performance.now()});
 ...
 await restore(fixture());await swipe();
 ...
 const events=await page.evaluate(()=>window.scoreEvents);assert.deepEqual(events.map(e=>e.id),['bingo',...Array(5).fill('face-value')]);assert.equal(events[0].tiles.length,5);assert.equal(events[0].active.length,0);assert.deepEqual(events.slice(1).map(e=>e.payout),['+5','+19','+23','+1','+4']);assert.deepEqual(events.slice(1).map(e=>e.active),Array.from({length:5},(_,i)=>[`cell-${i+1}`]));assert.equal((await read()).score,52);assert.equal(await page.locator('.stamped').count(),5);
 await restore(fixture(['bingo','face-value','single-digits:1']));await swipe();await page.waitForFunction(()=>window.scoreEvents.length>=9&&!document.querySelector('.scoring-rack')&&document.querySelector('#balls .ball:not([disabled])'));
 assert.equal((await read()).score,55);assert.deepEqual((await page.evaluate(()=>window.scoreEvents)).map(e=>e.id),['bingo','face-value','single-digits:1','face-value','face-value','face-value','single-digits:1','face-value','single-digits:1']);
 ...
 const old=fixture(['bingo','single-digits:1']);old.jokerVersion=3;await restore(old);assert.deepEqual((await read()).jokers,['bingo','face-value','single-digits:1']);
